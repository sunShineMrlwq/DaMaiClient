    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.bt_sendMessage://发消息
                ...
                if(null!=user){
                    Log.i("UserInfoActivity", "获取用户为" + user.userName);
                    Chat chat = AnIMManger.getInstance(this).addChat(user.clientId);//添加一个聊天对象
                    AnIMManger.getInstance(this).notifyChatUpdated();
                    i.putExtra("targetClientId", user.clientId);
                    Bundle b=new Bundle();
                    b.putSerializable(Constant.INTENT_EXTRA_KEY_CHAT,chat);
                    i.putExtras(b);
                }else{
                    Log.i("UserInfoActivity","通过UserClientId创建聊天记录");
                    String  UserClientId=   getIntent().getStringExtra("UserClientId");
                    Chat chat = AnIMManger.getInstance(this).addChat(UserClientId);//添加一个聊天对象
                    AnIMManger.getInstance(this).notifyChatUpdated();
                    i.putExtra("targetClientId",UserClientId);
                    Bundle b=new Bundle();
                    b.putSerializable(Constant.INTENT_EXTRA_KEY_CHAT, chat);
                    i.putExtras(b);
                }
