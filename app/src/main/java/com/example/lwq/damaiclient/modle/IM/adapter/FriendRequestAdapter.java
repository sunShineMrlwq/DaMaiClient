			...
			public void onClick(View v) {
				UserManager.getInstance(ct).approveFriendRequest(friendRequests.get(position), new IAnSocialCallback() {
					...
					public void onSuccess(JSONObject jsonObject) {
						...
						finalHolder.tv_addfriend.setClickable(false);
						//接受好友请求，就是加为好友,先获取好友对象，再加好友，后跳转好友信息页面
						 UserManager.getInstance(ct).fetchUserInfoByClientId(friendRequests.get(position).fromUserClientId, null, new UserManager.AddFriendsrCallback() {
							 @Override
							 public void onFinish(User user) {
								 UserManager.getInstance(ct).addFriendRemote(user, new UserManager.AddFriendsrCallback() {
									 @Override
									 public void onFinish(User user) {
										 try {
											 ...
											 AnIMManger.getInstance(ct).getAnIM().sendBinary(user.clientId, new byte[1], Constant.FRIEND_REQUEST_TYPE_SEND, c_data);
										 ...
										 i.putExtras(b);
										 i.putExtra("UserClientId",friendRequests.get(position).fromUserClientId);
