...
import com.example.lwq.damaiclient.widget.ListViewSwipeGesture;
...
public class ChatMessageActivity extends BaseActivity implements View.OnClickListener,Observer {
    ...
    public void initData(){
        mChat = (Chat) getIntent().getExtras().getSerializable(Constant.INTENT_EXTRA_KEY_CHAT);//添加一个聊天对象
        ...
        final ListViewSwipeGesture touchListener = new ListViewSwipeGesture(listview_chatmessage, swipeListener, this);
        touchListener.SwipeType	=	ListViewSwipeGesture.Dismiss;    //设置两个选项列表项的背景
        listview_chatmessage.setOnTouchListener(touchListener);

        fillLocalData(mChat.getFromTable());
    }
    //发送信息
    ...
    public IMMessage tosendMessage(IMMessage message){
        message.chat = mChat.getFromTable();
    ...
    public void appendUpdateMessage(IMMessage msg){
       if(TextUtils.equals(mChat.targetClientId,msg.chat.targetClientId)) {
            Log.i("chatActvityUpMessage","哈哈哈哈");
            int index=  chatMessageAdapter.addMessage(msg);
            listview_chatmessage.setSelection(index);
        }

    }
    //获取本地储存的数据
    public void fillLocalData(Chat chat){
        Log.i("chatID=====",chat.getId()+"");
        AnIMManger.getInstance(this).getMessageByChat(chat, new AnIMManger.GetMessageCallback(){
            @Override
    ...
    ListViewSwipeGesture.TouchCallbacks swipeListener = new ListViewSwipeGesture.TouchCallbacks() {
        ...
        public void FullSwipeListView(int position) {
            Toast.makeText(ChatMessageActivity.this, "Action_2", Toast.LENGTH_SHORT).show();
